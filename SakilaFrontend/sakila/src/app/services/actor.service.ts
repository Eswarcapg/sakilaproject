...
export class ActorService {
  private apiUrl = 'http://localhost:8081/api/v1/admin/actors/post';
  private baseUrl = 'http://localhost:8081/api/v1/public/actors';
  ...
  updateActorLastName(actorId: number, newLastName: string): Observable<string> {
    const url = `http://localhost:8081/api/v1/admin/actors/update/lastname/${actorId}`;
  ...
  updateActorFirstName(actorId: number, newFirstName: string): Observable<string> {
    const url = `http://localhost:8081/api/v1/admin/actors/update/firstname/${actorId}`;
    const requestBody = newFirstName ;
    return this.http.put(url, requestBody, { responseType: 'text' }).pipe(
      map(() => 'First Name Updated Successfully')
    );
  }
  assignFilmToActor(actorId: number, film: Film): Observable<string> {
    const url = `http://localhost:8081/api/v1/admin/actors/${actorId}/film`;
