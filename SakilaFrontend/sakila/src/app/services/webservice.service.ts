...
export class WebserviceService {
       ...
       searchByName(title: string): Observable<any> {
              ...
              const url = `http://localhost:8081/api/v1/films/public/title/${title}`;
              return this.http.get(url);
       }
       searchByGenre(genre: string): Observable<any> {

              const url = `http://localhost:8081/api/v1/films/public/category/${genre}`;
              return this.http.get(url);
       }
       searchByRating(rating: string): Observable<any> {

              const url = `http://localhost:8081/api/v1/films/public/rating/${rating}`;
              return this.http.get(url);
       }
       ...
       findFilmById(id: number): Observable<any> {
              const url = `http://localhost:8081/api/v1/films/public/${id}/actors`;
              return this.http.get(url);
       }
       searchById(id: number): Observable<any> {

              const url = `http://localhost:8081/api/v1/customer/customer/${id}`;

              return this.http.get(url);
       }
       searchRentalByCustomerId(id: number): Observable<any> {
              const url = `http://localhost:8081/api/v1/customer/rental/customer/${id}`;
              return this.http.get(url);
       }
}
